...
```{r init_data}
polls <- read.csv("revised_polls.csv") %>% 
...
polls <- polls[polls$Start.Date >= as.Date("2020-03-01", format = "%Y-%m-%d") & polls$Start.Date <= currentDate, ]
...
z.test.method <- function(state, date, size, population, biden, trump) {
  ...
  if(numBlocks == 0){
    ...
    #print(paste(state[1], block.final$beta[1], block.final$se, sum(block.finaldf$size), sep = ", "))
...
senate_states <- levels(as.factor(polls$State))
...
results <- left_join(results, demographics, by = "StateName") %>% 
 transmute(
    StateName = StateName,
    StateMean = StateMean,
    Variance = Variance + ((0.6 * (CoVI + 2.06))/100)^2 + (((as.numeric(as.Date("2020-11-03") - currentDate))/7)^(1/2)/400)^2,
    ElectoralVotes = ElectoralVotes,
    BPI = BPI
  ) %>% filter(StateName %in% senate_states)
